...
author: "Andres Calvo <ac228>" # YOU SHOULD EDIT THIS
date: 2024-01-19
week: 2
categories: [Lab]
...
```{julia}
df = vcat(annual_data...)
# Why do we use the "..." in the previous line?
first(df, 5)
```
...
Create a subset for March 2020
```{julia}
t_start = Dates.DateTime(2020, 3, 1, 0)
t_end = Dates.DateTime(2020, 3, 30, 23)
...
It can be concluded that there are not big differences between the day grouping to the trend found by grouping by month
```{julia}
df[!, :day] = Dates.dayofyear.(df.datetime)
dropmissing!(df, :lsl)
df_byday = groupby(df, :day)
df_climaday =  combine(df_byday, :lsl => mean => :lsl_avg)

plot(
    df_climaday.day,
    df_climaday.lsl_avg;
    xticks = [31,60,90,120,151,181,212,243,273,304,334,365],
